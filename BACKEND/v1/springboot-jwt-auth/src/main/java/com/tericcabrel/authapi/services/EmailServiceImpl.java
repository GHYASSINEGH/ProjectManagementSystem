...
import static com.tericcabrel.authapi.utils.EmailUtils.*;
...
public class EmailServiceImpl implements EmailService {
    ...
    @Override
    @Async
    public void sendSimpleMailMessage(String name,String password, String to, String token) {
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            message.setSubject(NEW_USER_ACCOUNT_VERIFICATION);
            message.setFrom(fromEmail);
            message.setTo(to);
            message.setText(getEmailMessage(name,password, host, token));
    ...
    @Override
    @Async
    public void sendSimpleMailTaskNotification(String name, String title, String description, String to, Integer userId) {
        try {
            SimpleMailMessage message = new SimpleMailMessage();
            message.setSubject(NEW_USER_ACCOUNT_VERIFICATION);
            message.setFrom(fromEmail);
            message.setTo(to);
            message.setText(getEmailTask(name,title,description, userId));
            emailSender.send(message);
        } catch (Exception exception) {
            System.out.println(exception.getMessage());
            throw new RuntimeException(exception.getMessage());
    ...
    @Override
    @Async
    public void sendMimeMessageWithAttachments(String name,String password, String to, String token) {
        try {
            ...
            helper.setTo(to);
            helper.setText(getEmailMessage(name,password, host, token));
            //Add attachments
    ...
    @Override
    @Async
    public void sendMimeMessageWithEmbeddedFiles(String name,String password, String to, String token) {
        try {
            MimeMessage message = getMimeMessage();
            MimeMessageHelper helper = new MimeMessageHelper(message, true, UTF_8_ENCODING);
            helper.setPriority(1);
